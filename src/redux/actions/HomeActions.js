...
import { getList } from '../../services/rickAPI'
...
const GetCharactersList = (page) => async (dispatch) => {
  ...
  try {
    const result = await getList('character', page)
...
const GetLocationsList = (page) => async (dispatch) => {
  ...
  try {
    const result = await getList('location', page)
...
const GetEpisodesList = (page) => async (dispatch) => {
  ...
  try {
    const result = await getList('episode', page)
